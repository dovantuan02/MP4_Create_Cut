...
// Given a track, make an encrypted clone of it in the dest. file
                            ...
                            MP4TrackId dstHintTrackReferenceTrack)
    {
        ...
        // MP4Duration editsDuration = MP4GetTrackDuration(srcFile, srcTrackId);
        ...
        while (true) {
            ...
            if (viaEdits) {
                sampleId = MP4GetSampleIdFromEditTime(
                               ...
                               NULL,
                               &sampleDuration);
                MP4Duration sampleError = editsDuration - sampleDuration;

                // printf("Sample Duration : %d - when: %d\n", sampleDuration, when);
                ...
                when += sampleDuration;
                uint32_t sampleErr = when + (sampleDuration);
                printf("When and sampleDuration : %d - editsDuration: %ld\n", sampleErr, editsDuration);
                
                if (sampleErr > editDuration) {
                    printf("Sample Err: %ld\n", sampleError);
                    break;
                }
            } else {
                sampleId++;
