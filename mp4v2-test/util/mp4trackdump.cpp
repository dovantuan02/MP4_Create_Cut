...
char* ProgName;
char* Mp4PathNameTrackDump;
char* Mp4FileNameTrackDump;
...
static void DumpTrack ( MP4FileHandle mp4file, MP4TrackId tid )
{
    ...
    printf( "mp4file %s, track %d, samples %d, timescale %d\n",
            Mp4FileNameTrackDump, tid, numSamples, timescale );
...
extern "C" int main( int argc, char** argv )
{
    ...
    Mp4PathNameTrackDump = argv[prog::optind++];

    char* lastSlash = strrchr( Mp4PathNameTrackDump, '/' );
    if ( lastSlash ) {
        Mp4FileNameTrackDump = lastSlash + 1;
    }
    else {
        Mp4FileNameTrackDump = Mp4PathNameTrackDump;
    }
    ...
    MP4FileHandle mp4File = MP4Read( Mp4PathNameTrackDump );
